...
3010 ADSBHUB$="A5D570|40.782088|-74.169950|5875|UAL1496|202|277|64|A320|N475UA|3373|UAL|6.19"
3020 NA=0:GET TIME TI$
3025 PSET(405,12),0:PRINT #1,TI$
3030 X=1:L=0:C=LEN(ADSBHUB$)
3040 N=INSTR(X,ADSBHUB$,"|"):IF N=0 AND X<C+1 THEN N=C+1
3050 K$=MID$(ADSBHUB$,X,N-X)
3055 IF LEN(K$)=0 THEN 3200
3060 IF L=0 THEN XA$(NA)=K$
3070 IF L=1 THEN XB#(NA)=(VAL(K$)*100.0-LA#*100.0)
3080 IF L=2 THEN XC#(NA)=(VAL(K$)*100.0-LO#*100.0)
3090 IF L=3 THEN XD(NA)=VAL(K$)
3100 IF L=4 THEN XE$(NA)=K$
3110 IF L=5 THEN XF(NA)=VAL(K$)
3120 IF L=6 THEN XG(NA)=VAL(K$)
3130 IF L=7 THEN XH(NA)=VAL(K$)
3140 IF L=8 THEN XI$(NA)=K$
3150 IF L=9 THEN XJ$(NA)=K$
3160 IF L=10 THEN XK(NA)=VAL(K$)
3170 IF L=11 THEN IF LEN(K$)>8 THEN XL$(NA)=LEFT$(K$,8) ELSE XL$(NA)=K$
3180 IF L=12 THEN XM#(NA)=VAL(K$)
3190 X=N+1:L=L+1
3200 IF X<C+1 THEN 3040
3210 IF XM#(NA)<ZO THEN GOSUB 3500
3220 NA=NA+1
3230 RETURN
