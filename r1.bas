...
2040 ZX#=0.69172*210.0*COS(LA#*180/3.1415926)/ZO
2050 ZY#=0.69*210.0/ZO
2060 RETURN
...
3070 IF L=2 THEN XC#(NA)=VAL(K$)*100.0-LO#*100.0
...
3142 IF L=10 THEN XK(NA)=VAL(K$)
3144 IF L=11 THEN XL$(NA)=K$
3146 IF L=12 THEN XM#(NA)=VAL(K$)
3150 X=N+1:L=L+1
3160 IF X<C THEN 3040
3170 IF XM#(NA)<ZO THEN GOSUB 3500
3180 NA=NA+1
3190 RETURN
...
4010 SE$=XA$(SI)
4012 PSET (2,152),0:PRINT #1,STR$(XB#(SI))+" "+STR$(XC#(SI))
