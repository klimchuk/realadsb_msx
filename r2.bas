...
3035 SA$=STR$(LA#):IF LA#>0.0 THEN SA$=RIGHT$(SA$,LEN(SA$)-1)
3037 SO$=STR$(LO#):IF LO#>0.0 THEN SO$=RIGHT$(SO$,LEN(SO$)-1)
3040 U$="http://"+IP$+"/a?lat="+SA$+"&lon="+SO$+"&num=10"
3045 ON ERROR GOTO 3330
3050 OPEN "HTTP:U$" AS #2
...
3280 NA=NA+1
3285 IF NA=10 THEN CLOSE #2:GOTO 3300
3290 GOTO 3060 
3300 GET TIME TI$
3310 PSET(405,12),0:PRINT #1,TI$+" "
3320 RETURN
3330 PSET(405,12),0:PRINT #1,"ERROR: "+STR$(ERR)
3340 RETURN
...
3560 PUT SPRITE NA+10,(120+INT(XC#(NA)*ZX#),98-INT(XB#(NA)*ZY#)),10,NA+4
3570 RETURN
...
5000 REM METAR
5010 INTERVAL OFF
5020 FOR XA=0 TO 19:PUT SPRITE XA,(-32,-32),15,0:NEXT XA
5030 COLOR 10,1,4
...
5090 REM METAR$="2022/04/09 00:51"+CHR$(10)+"KEWR 090051Z 17006KT 10SM FEW060 14/M01 A2968 RMK AO2 PK WND 30028/1340 SLP088 T00891044"
5100 U$="http://"+IP$+"/metar?icao="+AI$
5102 L=0 
5105 ON ERROR GOTO 5300
5110 OPEN "HTTP:U$" AS #3
5120 IF EOF(3) THEN CLOSE #3:GOTO 5260
5130 LINE INPUT #3, METAR$
5140 X=1:C=LEN(METAR$)
5145 IF C=0 THEN 5120
5150 N=INSTR(X,METAR$,CHR$(10)):IF N=0 AND X<C THEN N=C
5160 REM Loop for long (>50) lines
5170 FOR M=0 TO (N-X)\50
5180 XL=N-X+1:IF XL>50 THEN XL=50
5190 PSET(30,40+L*10),0
5200 PRINT #1,MID$(METAR$,X,XL)
5210 X=X+XL:L=L+1
5220 NEXT M
5230 X=N+2
5240 IF X<C THEN 5150
5250 GOTO 5120
5260 B$=INKEY$
5270 IF B$=CHR$(27) THEN 530 
5280 GOTO 5260
5290 REM Error happened
5300 PSET(30,40+L*10),0
5310 PRINT #1,"ERROR: "+STR$(ERR)
5320 GOTO 5260
6000 REM List
6010 INTERVAL OFF
6020 FOR XA=0 TO 19:PUT SPRITE XA,(-32,-32),15,0:NEXT XA
6030 COLOR 10,1,4
